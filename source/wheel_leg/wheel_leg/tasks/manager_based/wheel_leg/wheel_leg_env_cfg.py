...
@configclass
class NoDrObservationsCfg(ObservationsCfg):
    @configclass
...
@configclass
class WheelLegNoDrEnvCfg(ManagerBasedRLEnvCfg):
    scene: WheelLegSceneCfg = WheelLegSceneCfg(num_envs=4096, env_spacing=2.5)
    commands: CommandsCfg = CommandsCfg()
    observations: NoDrObservationsCfg = NoDrObservationsCfg()
    actions: ActionsCfg = ActionsCfg()
...
class WheelLegEvalRoughEnvCfg(WheelLegEnvCfg):
    ...
    def __post_init__(self) -> None:
        ...
        self.events.reset_robot.params["pose_range"]["z"] = (0.00, 0.00)
